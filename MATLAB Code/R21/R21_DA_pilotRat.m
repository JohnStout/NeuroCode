...
%% run baseline - this code is incomplete. It should give us coherence thresholds
[] = R21_getBaseline();
...
loop_time = .84; % 0.84 is 50/60secs, to account for initial pause of 10sec .25; % minutes - note that this isn't perfect, but its a few seconds behind dependending on the length you set. The lag time changes incrementally because there is a 10-20ms processing time that adds up

% coherence thresholds
threshold.coherence_magnitude = 0.57; % coherence magnitude
threshold.coherence_duration  = 1.5;  % sec
...
% define for loop
looper = ceil((loop_time*60)/amountOfData); % N minutes * 60sec/1min * (1 loop is about .250 ms of data)

% define total loop time
total_loop_time = loop_time*60; % in seconds
...
for triali = 1:numTrials
    ...
    if delay_length > 1
        ...
        for i = 1:looper
            ...
            if coh(i) >= threshold.coherence_magnitude % this is set to >= because the looper is super specific (0.25 increment), so the values are continuously sampled, but discretely. Therefore, to improve detection, set to >=
                ...
                % are met
                if dur_sum >= threshold.coherence_duration
