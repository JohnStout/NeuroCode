...
for n = 1:numIter 
    ...
    for nIt2 = 1:numIter % this needs to change, it needs to be a permutative test, so prob 1000
        ...
        % train model separately on all classes
        model = cell(numClasses,1);
        for k=1:numClasses % so we'll build model on each of the three datasets
        ...
        % test classifier
        [predict_label{nIt}{nIt2}, accuracy{nIt}{nIt2}, p] = svmpredict(testLabel, testData, model{nIt}{nIt2},'-b 1');
